...
@MainActor final class Editor:ObservableObject {
    ...
    func perform(_ title:String, importing:Bool = false,_ operation:@escaping @Sendable () throws -> CGImage) {
        guard !busy else{return};busy=true;error=nil;progress=0;message=title
        ...
        worker=Task.detached(priority:.userInitiated) { let result=try operation();try Task.checkCancellation();return result }
        let current=worker!
        task=Task { do { let output=try await current.value;guard generation==id else{return}; if let image{history.append(image);if history.count>3{history.removeFirst()}};image=output;if importing{original=output;history=[]};adjustments=Adjustments();message="Готово · \(output.width) × \(output.height)";progress=1 } catch { if generation==id { if error is CancellationError{message="Обработка остановлена. Фото не изменено"}else{self.error=error.localizedDescription;message="Фото не изменено"} } };if generation==id{busy=false;endBackground()} }
    }
    ...
    func cancel(){worker?.cancel();message="Останавливаем после текущего участка…";endBackground()}
    func load(_ data:Data){perform("Открываем фотографию…", importing:true){try PhotoEngine.shared.decode(data)}}
